...
export function FocusTimer({ expanded = false }: { expanded?: boolean }) {
  ...
  return (
    <section className={`focus-timer focus-timer-complete${expanded ? ' expanded' : ''}`} aria-label="Temporizador de foco">
      ...
      <div className="timer-display" style={{ '--timer-progress': `${Math.round((1 - secondsLeft / (minutes * 60)) * 360)}deg` } as CSSProperties}><p className="eyebrow">{intention || 'Bloco de foco'}</p><strong>{formatTime(secondsLeft)}</strong><span>{sessions ? `${sessions} bloco${sessions === 1 ? '' : 's'} concluído${sessions === 1 ? '' : 's'} hoje` : `${minutes} minutos para uma coisa importante`}</span></div>
